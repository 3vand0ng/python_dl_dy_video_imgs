    ...
    for idx, img_url in enumerate(img_list):
        ...
        try:
            ...
            if img_resp.status_code == 200:
                ...
                print(
                    f"Failed to download image {idx+1}. Status: {img_resp.status_code}"
                )
...
def download_video(session, video_id):
    ...
    api_url = (
        f"https://api.amemv.com/aweme/v1/play/?video_id={video_id}&ratio=1080p&line=0"
    )
    ...
    with open(filename, "wb") as f:
        ...
        for data in video_resp.iter_content(block_size):
            ...
            if total_size > 0:
                ...
                sys.stdout.write(
                    f"\r[{'=' * percent}{' ' * (50 - percent)}] {downloaded}/{total_size} bytes"
                )
                sys.stdout.flush()
